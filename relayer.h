...
class relayer
{
	...
	private:
		...
		tcp::socket socketA;
		tcp::socket socketB;
		std::queue<std::pair<void*, size_t>> dataA;
		std::queue<std::pair<void*, size_t>> dataB;
		bool writingA, writingB;
		size_t bytesA, bytesB;

		void handle_read(void *buffer, bool from_A,
						 const boost::system::error_code &ec, size_t bytes_transferred);
		void handle_write(void *buffer, bool to_A,
						  const boost::system::error_code &ec, size_t bytes_transferred);
